...
- [~] **Rapordan odaklı blog yazıları** (IA §4.2): **#9 en iyi Türkçe LLM ✅** (`/dokumanlar/blog/turkce-icin-en-iyi-acik-kaynak-llm`); sırada #6 donanım seçimi, #5 LLM hazır mı. Tüm Türkçe içerik global `tr-humanizer` skill'iyle yazılır (doğal Türkçe + AI-tell temizliği; noktadan-sonra-bağlaç kuralı dahil).
